...
if(!isset($_SESSION['user_name'])){
 ...
 $result = display_cancelled_order();
                        ...
                        <?php
                            ...
                            $conn = mysqli_connect('localhost','root','','chappy_db');
                                ...
                                if(isset($_GET['search-order']))
                                {
                                    ...
                                    $user_id = $_SESSION['ID'];
                                    $query = "SELECT * FROM order_form WHERE CONCAT(custom_image, order_id, payer_name, shape, size, material, qty, processing, payment_total_amount, date_order) LIKE '%$filtervalue%' && customer_id = '$user_id' &&status = 'Cancelled by customer' || status = 'Cancelled by Admin' ";
